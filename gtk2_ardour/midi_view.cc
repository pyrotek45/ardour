...
MidiView::update_resizing (NoteBase* primary, bool at_front, double delta_x, bool relative, double snap_delta, bool with_snap)
{
	...
	for (auto & rd : _resize_data) {
		...
		if (!_on_timeline) {
			 ...
			 * clip-local tempo map.  We must snap the raw pixel position
			 * directly in that local space without adding the region's
			 * session-absolute offset, because adding it would shift the
			 * snap grid by the fractional beat offset of the clip's start
			 * position, causing snapping to land on wrong beats whenever
			 * the clip is not placed on a beat boundary. */
